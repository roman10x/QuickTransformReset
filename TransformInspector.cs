...
{
	static public TransformInspector m_instance;

	SerializedProperty m_mPos;
	SerializedProperty m_mRot;
	SerializedProperty m_mScale;
	...
	void OnEnable ()
	{
		...
		if (this)
		{
			try
			{
				...
				m_mPos = so.FindProperty("m_LocalPosition");
				m_mRot = so.FindProperty("m_LocalRotation");
				m_mScale = so.FindProperty("m_LocalScale");
			}
			catch { }
		}
	}
	...
	void DrawPosition ()
	{
		...
		EditorGUILayout.PropertyField(m_mPos.FindPropertyRelative("x"));
		EditorGUILayout.PropertyField(m_mPos.FindPropertyRelative("y"));
		EditorGUILayout.PropertyField(m_mPos.FindPropertyRelative("z"));
		GUILayout.EndHorizontal();

		if (reset) m_mPos.vector3Value = Vector3.zero;
	}
	...
	void DrawScale ()
	{
		GUILayout.BeginHorizontal();
		{
			...
			EditorGUILayout.PropertyField(m_mScale.FindPropertyRelative("x"));
			EditorGUILayout.PropertyField(m_mScale.FindPropertyRelative("y"));
			EditorGUILayout.PropertyField(m_mScale.FindPropertyRelative("z"));

			if (reset) m_mScale.vector3Value = Vector3.one;
		}
		GUILayout.EndHorizontal();
	}

#region Rotation looks ugly, since there is no native support for quaternion property drawing
	enum Axes : int
	{
		None = 0,
		X = 1,
	...
	private void DrawRotation ()
	{
		GUILayout.BeginHorizontal();
		{
			...
			Axes changed = CheckDifference(m_mRot);
			...
			if (reset)
			{
				m_mRot.quaternionValue = Quaternion.identity;
			}
